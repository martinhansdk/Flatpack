...
namespace nester {
	...
	polygon_p NesterPart::toPolygon() const {
		return outer_ring->toPolygon();
	}
	...
	bool segmentsIntersect(point_t a1, point_t a2, point_t b1, point_t b2) {
		...
		if (d1 == 0.0 && onSeg(a1, b1, b2)) return true;
		if (d2 == 0.0 && onSeg(a2, b1, b2)) return true;
		if (d3 == 0.0 && onSeg(b1, a1, a2)) return true;
		if (d4 == 0.0 && onSeg(b2, a1, a2)) return true;
	...
	bool polygonsOverlap(const polygon_t& a, const polygon_t& b) {
		...
		if (ba.maxX < bb_b.minX || bb_b.maxX < ba.minX ||
		    ba.maxY < bb_b.minY || bb_b.maxY < ba.minY)
			return false;
	...
	double polygonMinDistance(const polygon_t& a, const polygon_t& b) {
		...
		for (int i = 0; i < na; i++) {
			...
			for (int j = 0; j < nb; j++) {
				...
				double d;
				d = ptSegDist(a1, b1, b2); if (d < minDist) minDist = d;
				d = ptSegDist(a2, b1, b2); if (d < minDist) minDist = d;
				d = ptSegDist(b1, a1, a2); if (d < minDist) minDist = d;
				d = ptSegDist(b2, a1, a2); if (d < minDist) minDist = d;
			}
		}
		return minDist;
	}
	...
	static transformer_t computePlacementTransformer(const polygon_t& outerPoly,
	                                                  const Placement& pl) {
		if (outerPoly.empty()) return transformer_t(1.0);
		polygon_t rotated = transformPolygon(outerPoly,
		                                    makeTransformation(pl.angle, 0.0, 0.0));
		BoundingBox rbb = computeBB(rotated);
		return makeTransformation(pl.angle,
		                          pl.x - (double)rbb.minX,
		                          pl.y - (double)rbb.minY);
	}
	...
	static vector<vector<HoleSlot>> buildHoleCandidates(
	...
	static vector<Placement> makeInitialPlacement(
	    const vector<NesterPart_p>& parts,
	    const vector<polygon_t>& origPolys,
	    double kerf_cm)
	{
		...
		for (size_t i = 0; i < parts.size(); i++) {
			...
			polygon_t rotated = transformPolygon(origPolys[i],
			                                    makeTransformation(pl.angle, 0.0, 0.0));
			BoundingBox rbb = computeBB(rotated);
	...
	static double computeEnergy(const vector<Placement>& placements,
		...
		for (size_t i = 0; i < placements.size(); i++) {
			if (placements[i].hostPartIndex != -1) continue;
			if (cachedPolys[i].empty()) continue;
			totalBB.join(computeBB(cachedPolys[i]));
			anyPart = true;
		}
		if (!anyPart) return 0.0;
	    ...
	    double kerf_cm)
	{
		...
		if (pl.hostPartIndex == -1) {
			...
			for (int j = 0; j < (int)placements.size(); j++) {
				if (j == idx) continue;
				if (placements[j].hostPartIndex != -1) continue;
				...
				if (polygonsOverlap(poly, other)) return false;
			...
			if (hostIdx < 0 || hostIdx >= (int)parts.size()) return false;

			auto holeParts = parts[hostIdx]->toHolePolygons();
			if (holeIdx < 0 || holeIdx >= (int)holeParts.size()) return false;
			if (!holeParts[holeIdx] || holeParts[holeIdx]->empty()) return false;

			// Transform the hole polygon using the host's placement.
			transformer_t hostT = computePlacementTransformer(origPolys[hostIdx],
			                                                   placements[hostIdx]);
			polygon_t placedHole = transformPolygon(*holeParts[holeIdx], hostT);

			// All vertices of the part must lie inside the hole.
			for (const point_t& p : poly) {
				if (!pointInPolygon(p, placedHole)) return false;
			}
			...
			for (int j = 0; j < (int)placements.size(); j++) {
				...
				if (placements[j].hostPartIndex != hostIdx ||
				    placements[j].hostHoleIndex != holeIdx) continue;
				const polygon_t& other = cachedPolys[j];
				if (other.empty()) continue;
				if (polygonsOverlap(poly, other)) return false;
				if (kerf_cm > 0.0 && polygonMinDistance(poly, other) < kerf_cm)
	...
	void Nester::setKerf(double k) {
		kerf_cm = k;
	}

	vector<Placement> Nester::getPlacements() const {
		return placements;
	}
	...
	void Nester::run(std::function<bool(int, int)> progress) {
			...
			for (int inner = 0; inner < INNER; inner++) {
				...
				if (r < 0.60) {
					...
					if (newPl.hostPartIndex == -1) {
						...
						uniform_int_distribution<int> holeDist(
						    0, (int)holeCandidates[idx].size() - 1);
						...
						if (!holeParts.empty() &&
						    slot.holeIndex < (int)holeParts.size() &&
						    holeParts[slot.holeIndex] &&
							...
							polygon_t placedHole = transformPolygon(
							    *holeParts[slot.holeIndex], hostT);
		...
		for (size_t i = 0; i < placements.size(); i++) {
			if (placements[i].hostPartIndex != -1) continue;
			if (bestCachedPolys[i].empty()) continue;
